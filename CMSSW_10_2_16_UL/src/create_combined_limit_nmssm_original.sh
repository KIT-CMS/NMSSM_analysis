...
for ERA in ${ERAS[@]}; do
    if [ "$CHANNEL" == "all" ]
        then
            MorphingMSSMvsSM --era=${ERA} --auto_rebin=1 --binomial_bbb=1 --analysis=nmssm --channel="mt et tt" --heavy_mass=${MASS}  --light_mass=${MASS_H2} --output_folder="output_"${ERA}"_"${CHANNEL}"_"${TRAINING_MASS}"_"${TRAINING_BATCH} --real_data=1 --training_mass=${TRAINING_MASS} --training_batch=${TRAINING_BATCH} --base_path=output/parametrized_nn_mH1000/uncert_shapes/synced_shapes/
        else
            MorphingMSSMvsSM --era=${ERA} --auto_rebin=1 --binomial_bbb=1 --analysis=nmssm --channel=${CHANNEL} --heavy_mass=${MASS} --light_mass=${MASS_H2} --output_folder="output_"${ERA}"_"${CHANNEL}"_"${TRAINING_MASS}"_"${TRAINING_BATCH} --real_data=1 --training_mass=${TRAINING_MASS} --training_batch=${TRAINING_BATCH} --base_path=output/parametrized_nn_mH1000/uncert_shapes/synced_shapes/
    fi
done

TARGET=output_combined_${CHANNEL}_${TRAINING_MASS}_${TRAINING_BATCH}_nmssm_${MASS}_${MASS_H2}/combined/cmb
## remove old files
...
for ERA in ${ERAS[@]}; do
    ...
    DATACARDDIR=output_${ERA}_${CHANNEL}_${TRAINING_MASS}_${TRAINING_BATCH}_nmssm_${MASS}_${MASS_H2}/${ERA}/cmb
...
combineTool.py -M T2W -o "ws.root"  --PO '"map=^.*/NMSSM_'${MASS}'_125_'${MASS_H2}'$:r_NMSSM_'${MASS}'_125_'${MASS_H2}'[0,-10,10]"' -i output_combined_${CHANNEL}_${TRAINING_MASS}_${TRAINING_BATCH}_nmssm_${MASS}_${MASS_H2}/combined/cmb/ -m ${MASS_H2} --parallel 1 -P HiggsAnalysis.CombinedLimit.PhysicsModel:multiSignalModel

combineTool.py -m $MASS_H2 -M AsymptoticLimits --rAbsAcc 0 --rRelAcc 0.0005 --redefineSignalPOIs r_NMSSM_${MASS}_125_${MASS_H2} -d output_combined_${CHANNEL}_${TRAINING_MASS}_${TRAINING_BATCH}_nmssm_${MASS}_${MASS_H2}/combined/cmb/ws.root --there -n ".NMSSM_"${MASS}"_125_"  --task-name NMSSM_${MASS}_125_${MASS_H2} --parallel 1 

combineTool.py -M CollectLimits output_combined_${CHANNEL}_${TRAINING_MASS}_${TRAINING_BATCH}_nmssm_${MASS}_${MASS_H2}/combined/cmb/higgsCombine*.root --use-dirs -o nmssm_${CHANNEL}_${TRAINING_MASS}_${TRAINING_BATCH}_${MASS}_${MASS_H2}.json

#PostFitShapesFromWorkspace -d output_combined_${CHANNEL}_${TRAINING_MASS}_${TRAINING_BATCH}_nmssm_${MASS}_${MASS_H2}/combined/${CHANNEL}/combined.txt.${CHANNEL} -w output_combined_${CHANNEL}_${TRAINING_MASS}_${TRAINING_BATCH}_nmssm_${MASS}_${MASS_H2}/combined/${CHANNEL}/ws.root -o output_combined_${CHANNEL}_${TRAINING_MASS}_${TRAINING_BATCH}_nmssm_${MASS}_${MASS_H2}/cmb/${CHANNEL}/prefitshape.root --freeze r_NMSSM_${MASS}_125_${MASS_H2}=0.01 --postfit
