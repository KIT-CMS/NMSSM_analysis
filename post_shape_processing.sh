...
do 
    ...
    do
        ...
        do
            if [ "$batch" -eq 1 ]; then
            echo "${era}-${ch}-${batch}"
            TAG=pNN_all_batches_mH1000_batch_${batch}
            mkdir -p output/${TAG}/uncert_shapes/
            procs="data emb ggh qqh vh tth zl ttl vvl ttt NMSSM_1000_125_60 NMSSM_1000_125_70 NMSSM_1000_125_75 NMSSM_1000_125_80"
            lms="60 70 75 80"
            fi 
            if [ "$batch" -eq 2 ]; then
            echo "${era}-${ch}-${batch}"
            TAG=pNN_all_batches_mH1000_batch_${batch}
            mkdir -p output/${TAG}/uncert_shapes/
            procs="data emb ggh qqh vh tth zl ttl vvl ttt NMSSM_1000_125_85 NMSSM_1000_125_90 NMSSM_1000_125_95 NMSSM_1000_125_100"
            lms="85 90 95 100"
            fi
            if [ "$batch" -eq 3 ]; then
            echo "${era}-${ch}-${batch}"
            TAG=pNN_all_batches_mH1000_batch_${batch}
            mkdir -p output/${TAG}/uncert_shapes/
            procs="data emb ggh qqh vh tth zl ttl vvl ttt NMSSM_1000_125_110 NMSSM_1000_125_120 NMSSM_1000_125_130 NMSSM_1000_125_150"
            lms="110 120 130 150"
            fi
            if [ "$batch" -eq 5 ]; then
            echo "${era}-${ch}-${batch}"
            TAG=pNN_all_batches_mH1000_batch_${batch}
            mkdir -p output/${TAG}/uncert_shapes/
            procs="data emb ggh qqh vh tth zl ttl vvl ttt NMSSM_1000_125_350 NMSSM_1000_125_400 NMSSM_1000_125_450 NMSSM_1000_125_500"
            lms="350 400 450 500"
            ...
            TAG=pNN_all_batches_mH1000_batch_${batch}
            mkdir -p output/${TAG}/uncert_shapes/
            procs="data emb ggh qqh vh tth zl ttl vvl ttt NMSSM_1000_125_550 NMSSM_1000_125_600 NMSSM_1000_125_650 NMSSM_1000_125_700"
            lms="550 600 650 700"
            fi
            if [ "$batch" -eq 7 ]; then
            echo "${era}-${ch}-${batch}"
            TAG=pNN_all_batches_mH1000_batch_${batch}
            mkdir -p output/${TAG}/uncert_shapes/
            procs="data emb ggh qqh vh tth zl ttl vvl ttt NMSSM_1000_125_750 NMSSM_1000_125_800 NMSSM_1000_125_850"
            lms="750 800 850"
            fi
            for proc in $procs 
            do
                echo "${era}-${ch}-${batch}-${proc}"
