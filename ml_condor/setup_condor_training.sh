...
if [[ -f "${OUTPUT_PATH}/lockfile.txt" ]] && [[ -f "${LOG_FILE}" ]]; then
  ...
  if [[ ${FORCE} == "True" ]]; then
    ...
    echo "There is already a training running for ${ERA}_${CHANNEL} (${OLD_JOB_ID})."
  ...
  echo "This is the lock file for the dataset ${ERA}_${CHANNEL}.
...
./ml_condor/write_condor_submission.sh ${ERA} ${CHANNEL} ${CALC} ${FOLDER} $
...
while ${condor_working}; do
  ...
  if [[ ${START_ID} -ne ${CURRENT_ID} ]] || [[ ! -f ${LOG_FILE} ]] || grep "Job was aborted" ${LOG_FILE} | grep ${CURRENT_ID}; then
    ...
    echo "."
    echo "Condor job with parameters ${ERA}_${CHANNEL} (${START_ID}) or its logfile was changed/deleted during runtime"
    echo "Logfiles are in ${OUTPUT_PATH}/${LOG_DIR}/"
    exit 1
  elif grep -q "Job was held" ${LOG_FILE}; then
    echo "."
    echo "Condor job with parameters ${ERA}_${CHANNEL}_${CALC} (${START_ID}) was held"
    echo "Logfiles are in ${OUTPUT_PATH}/${LOG_DIR}/"
    exit 1
  elif grep -q "(return value 0)" ${LOG_FILE}; then
    echo "."
    echo "Condor job with parameters ${ERA}_${CHANNEL}_${CALC} (${START_ID}) was completed succesfully"
    condor_working=false
  elif grep -q "(return value 1)" ${LOG_FILE}; then
    echo "."
    echo "Condor job with parameters ${ERA}_${CHANNEL}_${CALC} (${START_ID}) encountered an error"
    condor_working=false
  else
    if grep -q "Job executing on host" ${LOG_FILE} && ${job_waiting}; then
      echo "."
      echo "The job with parameters ${ERA}_${CHANNEL}_${CALC} (${START_ID}) started execution"
      job_waiting=false
    else
      echo -n "."
    fi
...
mv condor_output_${ERA}_${CHANNEL}/* ${OUTPUT_PATH}
rm -r condor_output_${ERA}_${CHANNEL}
